function [nU_BS_0,nU_BS_1,nU_BS_2,Pr_dB, power_dB, L_dB, d, association_matrix, downlink_tier_per_user, BS_associated] = downlinkUserAssociation(U, nU, Pt_dB, nBS_0, nBS_1, nBS_2, BSType, BSLocation, blocking_prob, bias0, bias1, bias2)
...
for i = 1 : nU
    ...
    L_dB(i, :) = getPathLoss(BSType, d(i, :), blocking_prob);
    ...
    bias_array = [getBiasFactor(0, bias0, bias1, bias2).*ones([1 nBS_0])  getBiasFactor(1, bias0, bias1, bias2).*ones([1 nBS_1])   getBiasFactor(2, bias0, bias1, bias2).*ones([1 nBS_2])];
    ...
    %Remove the bias
    power_dB(i) = power_dB(i) - getBiasFactor(getTier(BS_associated(i),nBS_0, nBS_1, nBS_2),bias0, bias1, bias2);
